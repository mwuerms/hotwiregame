...
ISR(TIMER0_OVF_vect)
{
    ...
    if(timer_ms_tick.repeated_event.event) {
        if(timer_ms_tick.repeated_event.cnt_down_ms == 0) {
...
void timer_start_ms_repeated_event(uint16_t ms_cnt_dn, uint8_t timer_event) {
    uint8_t sr = SREG;
    cli();

    timer_ms_tick.repeated_event.cnt_down_ms = ms_cnt_dn;
    timer_ms_tick.repeated_event.cnt_down_ms_reload = ms_cnt_dn;
    timer_ms_tick.repeated_event.event = timer_event;

    SREG = sr;
}

void timer_stop_ms_repeated_event(void) {
    uint8_t sr = SREG;
    cli();

    timer_ms_tick.repeated_event.cnt_down_ms = 0;
    timer_ms_tick.repeated_event.cnt_down_ms_reload = 0;
    timer_ms_tick.repeated_event.event = 0;
